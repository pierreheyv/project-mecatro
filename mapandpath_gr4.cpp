...
void update_destlist(Map* mymap, Path* simupath)//adapth mymap with the best path and total expected points in it variable have to be initialized before
{
        ...
        for(int i=0; i < NNODE; i++)
        {
            if (psavepath->visited[i] == 0)//si le noeud n'a pas encore été visité et est atteignable à temps => si tps d'aller au noeud + tps d'action au noeud + temps de retour à la base est < temps restant
            {
                ...
                if ((mymap->dist[simupath->actual_node][i] + mymap->dist[0][i]+ mymap->node[i][4]) < simupath->timeleft)
                {
                    ...
                    update_destlist(mymap, pnewpath);
                }
                else
                {
                    newpath.visited[i] = 1;//set analysing node as visited
    ...
    {
        if (mymap->mypath->totalPoints < simupath->totalPoints || (mymap->mypath->totalPoints == simupath->totalPoints && mymap->mypath->timeleft < simupath->timeleft)) //to be changed
        {
            mymap->mypath->objnb = simupath->objnb;
            mymap->mypath->totalPoints = simupath->totalPoints;
            mymap->mypath->timeleft = simupath->timeleft;
            for(int i = 0; i < simupath->objnb; i++)
...
Map* initmap()
{
    ...
    readmap(mymap);
    initpath(mymap);
    mymap->mypath->actual_node = BASENODE;
...
void load_obstacles(Map* mymap)
{
    ...
    //recifs
    obstacles[0][0]  = 1    ; obstacles[0][1]  = -0.6   ; obstacles[0][2]  = 0.33;
    obstacles[1][0]  = 0.8  ; obstacles[1][1]  = -0.6   ; obstacles[1][2]  = 0.23;
    obstacles[2][0]  = 0.7  ; obstacles[2][1]  = -0.6   ; obstacles[2][2]  = 0.19;

    obstacles[3][0]  = 1    ; obstacles[3][1]  = 0      ; obstacles[3][2]  = 0.33;
    obstacles[4][0]  = 0.8  ; obstacles[4][1]  = 0      ; obstacles[4][2]  = 0.23;
    obstacles[5][0]  = 0.7  ; obstacles[5][1]  = 0      ; obstacles[5][2]  = 0.19;

    obstacles[6][0]  = 1    ; obstacles[6][1]  = 0.6    ; obstacles[6][2]  = 0.33;
    obstacles[7][0]  = 0.8  ; obstacles[7][1]  = 0.6    ; obstacles[7][2]  = 0.23;
    obstacles[8][0]  = 0.7  ; obstacles[8][1]  = 0.6    ; obstacles[8][2]  = 0.19;

    //bancs de sable longs
    obstacles[9][0]  = -1   ; obstacles[9][1]  = -0.3   ; obstacles[9][2]  = 0.25;
    obstacles[10][0] = -0.8 ; obstacles[10][1] = -0.3   ; obstacles[10][2] = 0.15;
    obstacles[11][0] = -1   ; obstacles[11][1] = -0.5   ; obstacles[11][2] = 0.33;
    obstacles[12][0] = -1   ; obstacles[12][1] = -0.7   ; obstacles[12][2] = 0.33;
    obstacles[13][0] = -1   ; obstacles[13][1] = -0.9   ; obstacles[13][2] = 0.33;
    obstacles[14][0] = -1   ; obstacles[14][1] = -1.1   ; obstacles[14][2] = 0.33;
    obstacles[15][0] = -1   ; obstacles[15][1] = -1.3   ; obstacles[15][2] = 0.33;

    obstacles[16][0] = -1   ; obstacles[16][1] = 0.3    ; obstacles[16][2] = 0.25;
    obstacles[17][0] = -0.8 ; obstacles[17][1] = 0.3    ; obstacles[17][2] = 0.15;
    obstacles[18][0] = -1   ; obstacles[18][1] = 0.5    ; obstacles[18][2] = 0.33;
    obstacles[19][0] = -1   ; obstacles[19][1] = 0.7    ; obstacles[19][2] = 0.33;
    obstacles[20][0] = -1   ; obstacles[20][1] = 0.9    ; obstacles[20][2] = 0.33;
    obstacles[21][0] = -1   ; obstacles[21][1] = 1.1    ; obstacles[21][2] = 0.33;
    obstacles[22][0] = -1   ; obstacles[22][1] = 1.3    ; obstacles[22][2] = 0.33;

    //bancs de sable coins
    obstacles[23][0] = 0.8  ; obstacles[23][1] = -1.5   ; obstacles[23][2] = 0.43;
    obstacles[24][0] = 0.8  ; obstacles[24][1] = 1.5    ; obstacles[24][2] = 0.43;

    //pitits ilôts de sable
    obstacles[25][0]=-0.661 ; obstacles[25][1] = -0.308 ; obstacles[25][2] = 0.18;
    obstacles[26][0]=-0.661 ; obstacles[26][1] = 0.308  ; obstacles[26][2] = 0.18;
